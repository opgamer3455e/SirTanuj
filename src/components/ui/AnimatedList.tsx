...
const AnimatedItem: React.FC<AnimatedItemProps> = ({ children, delay = 0, index, onMouseEnter, onClick }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.4, delay: delay * 0.05, type: "spring", stiffness: 300, damping: 24 }}
      ...
      <div ref={listRef} className={`scroll-list ${!displayScrollbar ? 'no-scrollbar' : ''}`} onScroll={handleScroll}>
        <div className="flex flex-col gap-4">
          {items.map((item, index) => (
            ...
            >
              <div className={`animated-list-item ${selectedIndex === index ? 'selected ring-2 ring-[#00A699] rounded-3xl transition-shadow shadow-[0_0_20px_rgba(0,166,153,0.3)]' : ''} ${itemClassName}`}>
